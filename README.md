...
First, we need to create a self-contained json with all of the necessary versions to detokenize the code and execute. We split this portion explicitly because it is not possible to run the code and install all the libraries required to tokenize the Java and Python programs using a supercomputer. Thus, we do it elsewhere and create the resulting json file which can be used to generate results. 
