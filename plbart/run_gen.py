...
def main():
    ...
    if args.do_generate:
        ...
        if os.path.isfile(file):
            ...
            eval_examples, eval_data = load_and_cache_gen_data(
                args, '/home/$user/program_repair/CodeNet/data/java/processed/src_eval.java-java.java,/home/$user/program_repair/CodeNet/data/java/processed/tgt_eval.java-java.java', pool, tokenizer, 'eval', only_src=True, is_sample=False
            )
            logger.info("  Num examples = %d", len(eval_examples))
            logger.info("  Batch size = %d", args.eval_batch_size)
            eval_sampler = SequentialSampler(eval_data)
